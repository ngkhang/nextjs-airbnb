...
const FooterDefault = () => {
  return (
    <div className='bg-[#F7F7F7]'>
      {/* Main Footer */}
      <div className='container grid lg:grid-cols-3 lg:gap-2'>
      ...
      <div className='container py-6'>
        <div className='flex-col items-center justify-between text-sm md:flex xl:flex-row'>
          ...
          <div className='items-center lg:flex xl:order-1'>
            <p>&copy; {Footer.sub.copyright}</p>
            <ul className='flex flex-wrap'>
              {Footer.sub.others.map((item) => (
                <div key={item.key} className='flex items-center'>
                  <span className={cn('px-2 py-1 text-lg', item.key === 0 && 'hidden')}>·</span>
